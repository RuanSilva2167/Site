<html lang="pt-br">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="css/bootstrap.css">
    <link rel="stylesheet" href="css/css/styles.css">
    <link rel="stylesheet" href="css/css/css/bootstrap.css">
    <link rel="stylesheet" href="css/bootstrap.css">
    <link rel="stylesheet" href="css/stylecomplementar.css">
    <script src="js/mostrar.js" type="text/javascript"></script>
    <title>Grade Curricular</title>
    <link rel="shortcut icon" href="uepa.ico">
</head>

<body>
    <ul>
        <li> <img src="img/uepa-escudo80.png" alt="Logo da uepa" img-responsive img-centered> </li>

        <li><a href="https://www.uepa.pa.gov.br/">Site da Instituição</a></a>
        </li>
        <li><a href="http://ccnt.uepa.br/">Site do Centro de Ciencias Naturais e Tecnologia </a></a>
        </li>


    </ul>
    <header>
        <img src="img/logo-curso.png" alt="">
    </header>
    <?php 
     include("menu.php");
     ?>

    <h2>Grade Curricular do Curso</h2>

    <button type="button " class="action-bnt " onclick="ss() ">
                   1º Semestre
            </button>
    <div class="ss">
        <table>
            <th>Núcleo</th>
            <th>Disciplina</th>
            <th>Carga Horária</th>
            <tr>
                <td>DEMEI</td>
                <td>Matemática Discreta</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td> Fundamentos de Sistemas de Informação</td>
                <td>60h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td> Fundamentos de Sistemas Operacionais</td>
                <td>60h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Linguagens Formais</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Programação Estruturada</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Inglês Instrumental</td>
                <td>40h</td>
            </tr>
        </table>
    </div>
    <button type="button " class="action-bnt " onclick="ss1() ">
               2o Semestre
            </button>

    <div class="ss1 ">
        <table>
            <th>Núcleo</th>
            <th>Disciplina</th>
            <th>Carga Horária</th>
            <tr>
                <td>DEMEI</td>
                <td>Estatística Aplicada a Informática</td>
                <td>60h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Análise Orientada à Objetos</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Modelagem de Dados</td>
                <td>60h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Estrutura de Dados, Pesquisa e Ordenação</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Programação Web</td>
                <td>80h</td>
            </tr>
        </table>
    </div>
    ]<button type="button " class="action-bnt " onclick="ss2() ">
                3o Semestre
                </button>
    <div class="ss2 ">
        <table>
            <th>Núcleo</th>
            <th>Disciplina</th>
            <th>Carga Horária</th>
            <tr>
                <td>DENG</td>
                <td>Fundamentos de Engenharia de Software</td>
                <td>60h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Programação Orientada a Objetos I</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Banco de Dados I</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td> Análise e Projeto de Algoritmos</td>
                <td>40h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td> Microcontroladores e Microprocessadores</td>
                <td>60h</td>
            </tr>
        </table>
    </div>

    <button type="button " class="action-bnt " onclick="ss3() ">
               4o Semestre
            </button>
    <div class="ss3 ">
        <table>
            <th>Núcleo</th>
            <th>Disciplina</th>
            <th>Carga Horária</th>
            <tr>
                <td>DENGh</td>
                <td>Programação Orientada a Objetos II</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Análise e Projeto de Software</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Banco de Dados II</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td> Sistemas Multirobóticos</td>
                <td>60h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Projeto Integrado I (Elaboração de Projeto de Pesquisa – TCC)</td>
                <td>80h</td>
            </tr>
        </table>
    </div>

    <button type="button " class="action-bnt " onclick="ss4() ">
               5o Semestre
            </button>
    <div class="ss4 ">
        <table>
            <th>Núcleo</th>
            <th>Disciplina</th>
            <th>Carga Horária</th>
            <tr>
                <td>DENGh</td>
                <td>Processos de Desenvolvimento de Software</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Análise e Projeto de Software</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Gerência de Projetos</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Robótica Móvel</td>
                <td>60h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Cultura, Sociedade e Tecnologia</td>
                <td>60h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Fundamentos de Redes de Computadores</td>
                <td>80h</td>
            </tr>
        </table>
    </div>

    <button type="button " class="action-bnt " onclick="ss5() ">
                6o Semestre
            </button>
    <div class="ss5 ">
        <table>
            <th>Núcleo</th>
            <th>Disciplina</th>
            <th>Carga Horária</th>
            <tr>
                <td>DENG</td>
                <td>Fundamentos de Sistemas Embarcados</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Gestão de Infraestrutura de Redes</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Governança de TIC</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Programação Mobile</td>
                <td>60h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Compiladores e Interpretadores</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Projeto Integrado II – (TCC)</td>
                <td>80h</td>
            </tr>
        </table>
    </div>

    <button type="button " class="action-bnt " onclick="ss6()">
                7o Semestre
            </button>
    <div class="ss6 ">
        <table>
            <th>Núcleo</th>
            <th>Disciplina</th>
            <th>Carga Horária</th>
            <tr>
                <td>DENGh</td>
                <td>Fundamentos de Sistemas Embarcados</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Estimativa de Software</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Realidade Virtual</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Interação Humano Computador</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>DENG</td>
                <td>Optativa 1</td>
                <td>80h</td>
            </tr>
        </table>
    </div>

    <button type="button " class="action-bnt " onclick="ss7() ">
                  8o semestre
            </button>
    <div class="ss7 ">
        <table>
            <th>Núcleo</th>
            <th>Disciplina</th>
            <th>Carga Horaria</th>
            <tr>
                <td>Núcleo</td>
                <td>Teste de Software</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>Núcleo</td>
                <td>Análise e Visualização de Dados</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>Núcleo</td>
                <td>Sistemas Baseado em Software Livre - UNAMA</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>Núcleo</td>
                <td>Mineração de Dados</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>Núcleo</td>
                <td>Optativa 2</td>
                <td>80h</td>
            </tr>
            <tr>
                <td>Núcleo</td>
                <td>Trabalho de Conclusão de Curso</td>
                <td>--</td>
            </tr>
        </table>

    </div><br><br>

    <footer>

        <div class="container-center ">
            <ul class="social " id="asssbora ">
                <li><a href="https://pt-br.facebook.com/UepaOficial/">Facebook</a></li>
                <li><a href="https://twitter.com/uepa_pa">Twitter</a></li>
                <li>
                    <a href="https://www.youtube.com/user/ascomuepa">YouTube</a>
                </li>
                <li>
                    <a href="https://www.flickr.com/photos/biuepa/">Flickr</a>
                </li>
            </ul><br><br>
        </div>
        <div id="copy-area" class="coxinhadefrango">
            <div class="container">
                <div class="row">
                    <div class="col-md-12">
                        <p>Desenvolvido por <a href="#" target="_blank">Ruan Felipe, Eduardo Oliveira, Ana Caroline</a> &copy; 2021</p>
                    </div>
                </div>
            </div>
        </div>
    </footer>
</body>


</html>